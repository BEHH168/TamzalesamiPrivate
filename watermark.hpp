...
namespace watermark {
    ...
    inline void render() {
        ...
        ImVec2 ts1 = espFont ? espFont->CalcTextSizeA(text_sz, FLT_MAX, 0.f, "@gametocytes") : ImVec2(0, text_sz);
        ImVec2 ts2 = espFont ? espFont->CalcTextSizeA(text_sz, FLT_MAX, 0.f, "@pastercode") : ImVec2(0, text_sz);
        ImVec2 ts3 = iconFont ? iconFont->CalcTextSizeA(text_sz, FLT_MAX, 0.f, "\uf109") : ImVec2(0, text_sz);

        float content_w = pad_x + icon_sz + gap + ts1.x + sep_gap + sep_w + sep_gap + ts3.x + gap + ts2.x + pad_x;
        ...
        if (espFont) {
            ...
            dl->AddText(espFont, text_sz, ImVec2(cur_x, text_y), txt, "@gametocytes");
            cur_x += ts1.x;

            float sep_cy = ry + content_h * 0.5f;
            cur_x += sep_gap;
            dl->AddLine(ImVec2(cur_x, sep_cy - sep_h * 0.5f), ImVec2(cur_x, sep_cy + sep_h * 0.5f), IM_COL32(80, 80, 80, 255), sep_w);
            cur_x += sep_w + sep_gap;

            if (iconFont) {
                dl->AddText(iconFont, text_sz, ImVec2(cur_x, ry + (content_h - ts3.y) * 0.5f), IM_COL32(255, 222, 176, 255), "\uf109");
                cur_x += ts3.x;
                cur_x += gap;
            }

            dl->AddText(espFont, text_sz, ImVec2(cur_x, text_y), txt, "@pastercode");
        }
